...
const blogApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getAllBlog: builder.query<PaginatedResponseType<BlogType>, number>({
      query: (pageNumber) => `${apiPaths.blogUrl}?page=${pageNumber}`,
    ...
    getAllBlogCategory: builder.query<
      ...
      number
    >({
      query: (pageNumber) => `${apiPaths.blogCatrgoryUrl}?page=${pageNumber}`,
      providesTags: (response) =>
        response?.results
          ? [
              ...response.results.map(
