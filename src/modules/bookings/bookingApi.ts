      ...
      createBooking: builder.mutation<BookingDetailType, BookingFormType>({
        query: ({ ...payload }) => {
          ...
          if (payload.package != undefined)
            formData.append('package', payload.package as string);
          if (payload.fullName) formData.append('full_name', payload.fullName);
          if (payload.totalPrice)
            formData.append('total_price', payload.totalPrice);
          if (payload.departureDate)
            formData.append(
              'departure_date',
              payload.departureDate.toISOString()
            );
          if (payload.noOfTravelers != undefined)
            formData.append(
              'no_of_travellers',
              payload.noOfTravelers.toString()
            );
